...
if __name__=='__main__':
    ...
    caffe.set_mode_cpu()
    t1 = timeit.timeit(stmt='net.forward()',setup='from __main__ import net',number=1) # run once before timing to set up memory
    print 'time is: %fs' % t1

    caffe.set_device(7)  # if we have multiple GPUs, pick the first one
    caffe.set_mode_gpu()
    t2 = timeit.timeit(stmt='net.forward()',setup='from __main__ import net',number=1) # run once before timing to set up memory
    print 'time is: %fs' % t2
